from service import WoRMSService
import sys
import csv
import time
...
def clean_data_using_worms(
    ...
    # create log file 
    # sys.stdout = open(log_data_file_name,'w') 
    ...
    for original_record in input_data:
        ...
        print
        log_data.write('\n')
        log_data.write(timestamp('Reading input record {0:03d}. '.format(record_num)))
    ...
    # @OUT worms_lsid
        ...
        log_data.write("Trying WoRMs EXACT match for scientific name: '{0}'. ".format(original_scientific_name))
        ...
        if matching_worms_record is not None:
            log_data.write('WoRMs EXACT match was SUCCESSFUL. ')
        ...
        # otherwise try a fuzzy match
            ...
            if matching_worms_record is not None:
                log_data.write('WoRMs FUZZY match was SUCCESSFUL. ')
                timestamp('WoRMs FUZZY match was SUCCESSFUL.')
                worms_match_result = 'fuzzy'
            else:
                log_data.write('WoRMs FUZZY match FAILED. ')
    ...
    # @OUT rejected_data @FILE file:{rejected_data_file_name}
        ...
        if worms_match_result is None:
            ...
            log_data.write('REJECTED record {0:03d}. '.format(record_num))
    ...
    # @OUT cleaned_record
        ...
        if updated_scientific_name is not None:
            log_data.write("UPDATING scientific name from '{0}' to '{1}'. ".format(
        ...
        if updated_authorship is not None:
            log_data.write("UPDATING scientific name authorship from '{0}' to '{1}'. ".format(
    ...
    # @OUT cleaned_data  @FILE file:{cleaned_data_file_name}
        ...
        log_data.write('ACCEPTED record {0:03d}. '.format(record_num))
